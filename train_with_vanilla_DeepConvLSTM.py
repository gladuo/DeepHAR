...
import keras
from keras.layers import Input, Conv2D, LSTM, Dense, Dropout, Permute, Reshape, Flatten
...
from config_vanilla_DeepConvLSTM import *
...
def train():
    ...
    conv1 = Conv2D(filters=NUM_FILTERS, kernel_size=(1, FILTER_SIZE), strides=(1, 1), padding='valid',
                   activation='relu', kernel_initializer='normal', data_format='channels_last')(inputs)
    conv2 = Conv2D(filters=NUM_FILTERS, kernel_size=(1, FILTER_SIZE), strides=(1, 1), padding='valid',
                   activation='relu', kernel_initializer='normal', data_format='channels_last')(conv1)
    conv3 = Conv2D(filters=NUM_FILTERS, kernel_size=(1, FILTER_SIZE), strides=(1, 1), padding='valid',
                   activation='relu', kernel_initializer='normal', data_format='channels_last')(conv2)
    conv4 = Conv2D(filters=NUM_FILTERS, kernel_size=(1, FILTER_SIZE), strides=(1, 1), padding='valid',
                   activation='relu', kernel_initializer='normal', data_format='channels_last')(conv3)
    reshape1 = Reshape((FINAL_SEQUENCE_LENGTH, NUM_FILTERS * 1))(conv4)
    dropout1 = Dropout(DROPOUT_RATE)(reshape1)
    lstm1 = LSTM(NUM_UNITS_LSTM, return_sequences=True, implementation=2)(dropout1)
    dropout2 = Dropout(DROPOUT_RATE)(lstm1)
    lstm2 = LSTM(NUM_UNITS_LSTM, return_sequences=False, implementation=2)(dropout2)  # implementation=2 for GPU
